...
策略模型每回合生成受限 JSON：短的决策状态、缺失信息与一个 `clarify_user`、`search_case` 或 `Complete` 动作。适配器验证 JSON 后转换为 Huawei 评测器要求的原生工具调用，因此策略服务本身不需要支持 function calling。每条非基础设施失败的轨迹都会额外保存 `proactive_policy.decisions`，而 `run_config.json` 会记录适配器版本，避免与非 Proactive 轨迹混合断点续跑。
